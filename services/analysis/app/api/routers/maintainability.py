...
router = APIRouter(
    prefix="/api/v1/maintainability-findings", tags=["maintainability-findings"]
)
...
async def get_maintainability_metrics(
    ...
    return MaintainabilityMetricsResponse(
        maintainability_index=float(
            cast(float, metrics.get("maintainability_index", 100.0))
        ),
        technical_debt_hours=float(
            cast(float, metrics.get("technical_debt_hours", 0.0))
        ),
