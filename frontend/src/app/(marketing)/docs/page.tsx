...
/* ─── Icons ─────────────────────────────────────────────────── */
const Icons = {
  Search: () => <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>,
  ...
  Git: () => <svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><circle cx="18" cy="18" r="3"/><circle cx="6" cy="6" r="3"/><path d="M13 6h3a2 2 0 0 1 2 2v7"/><line x1="6" y1="9" x2="6" y2="21"/></svg>,
...
const DOC_CATEGORIES = [
  { icon: Icons.Book, title: "Getting Started", desc: "Platform overview, authentication, and setting up your first workspace." },
  { icon: Icons.Git, title: "Repository Analyzer", desc: "Connecting GitHub/GitLab, configuring rules, and understanding readiness scores." },
  { icon: Icons.Bot, title: "Agentic AI System", desc: "How to prompt agents, use the Orchestrator, and manage AI context." },
  { icon: Icons.Workflow, title: "Developer Productivity", desc: "Managing AI-enhanced notes, linking tasks to PRs, and organizing projects." },
  { icon: Icons.Code, title: "API Reference", desc: "REST API endpoints, WebSocket events, and authentication tokens." },
  { icon: Icons.LifeBuoy, title: "Troubleshooting", desc: "Common issues, observability logs, and how to read correlation IDs." },
];
      ...
      <header className={`fixed top-0 z-50 w-full transition-all duration-300 ${scrolled ? 'border-b border-white/5 bg-[#0a0a0f]/80 backdrop-blur-xl' : 'bg-transparent'}`}>
        <div className="container mx-auto flex h-20 items-center justify-between px-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link href={ROUTES.REGISTER} className="btn-shimmer rounded-xl px-5 py-2.5 text-sm font-semibold text-white">Dashboard</Link>
          </div>
        </div>
      </header>
        ...
        <section className="container mx-auto px-6 pt-12 pb-24 text-center">
          <div className="animate-fade-up mx-auto max-w-3xl">
            ...
            <p className="text-lg text-slate-400 mb-10">
              Explore our guides, API references, and architecture overviews to get the most out of the Kraivor Platform.
            </p>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {DOC_CATEGORIES.map((category, idx) => (
              <RevealSection key={idx} delay={`delay-[${idx * 100}ms]`}>
                <Link href="#" className="group block h-full rounded-2xl border border-white/10 bg-white/[0.02] p-8 backdrop-blur-md transition-all duration-300 hover:bg-white/[0.04] hover:border-[hsl(var(--primary))/40] hover:shadow-[0_8px_32px_-10px_hsla(var(--primary),0.2)] hover:-translate-y-1">
                  ...
                  <h3 className="mb-3 text-xl font-bold text-white group-hover:text-[hsl(var(--primary-light))] transition-colors">{category.title}</h3>
            ...
            <div className="rounded-3xl border border-white/10 bg-white/[0.01] p-8 md:p-12 backdrop-blur-sm">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                {[
                  ...
                  <Link key={i} href="#" className="group flex items-center justify-between p-4 rounded-xl border border-white/5 bg-white/[0.02] hover:bg-white/[0.05] hover:border-white/10 transition-colors">
                    <span className="text-sm text-slate-300 group-hover:text-white transition-colors">{article}</span>
        ...
        <section className="container mx-auto px-6 pb-12">
          <RevealSection>
            <div className="text-center">
              ...
              <Link href="#" className="inline-flex items-center gap-2 text-[hsl(var(--primary-light))] hover:text-white transition-colors font-medium">
