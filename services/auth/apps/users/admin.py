...
class UserAdmin(BaseUserAdmin):
    ...
    def session_count(self, obj):
        count = getattr(
            obj, "_session_count", obj.refresh_tokens.filter(revoked=False).count()
        )
        return format_html(
    ...
    def api_key_count(self, obj):
        count = getattr(
            obj, "_api_key_count", obj.api_keys.filter(revoked=False).count()
        )
        return format_html(
            '<a href="{}?user__id__exact={}">{}</a>',
    ...
    def get_queryset(self, request):
        ...
        return qs.annotate(
            _session_count=Count(
                "refresh_tokens", filter=Q(refresh_tokens__revoked=False)
            ),
    ...
    def revoke_all_sessions(self, request, queryset):
        ...
        count = RefreshToken.objects.filter(user__in=queryset, revoked=False).update(
            revoked=True
        )
