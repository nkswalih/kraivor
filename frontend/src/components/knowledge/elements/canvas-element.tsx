...
import type { CanvasElement, ArrowElementData, ShapeElementData, Position } from '@/types/knowledge';
...
interface Props {
  ...
  onArrowPointDragStart: (e: React.MouseEvent, elementId: string, pointType: 'start' | 'end' | 'intermediate' | 'control', pointIndex: number) => void;
...
function getHandleStyle(pos: typeof handlePositions[number], w: number, h: number) {
  ...
  switch (pos) {
    ...
    case 'n': return { top: -half, left: w / 2 - half, cursor: 'ns-resize' };
    case 'ne': return { top: -half, left: w - half, cursor: 'nesw-resize' };
    case 'e': return { top: h / 2 - half, left: w - half, cursor: 'ew-resize' };
    case 'se': return { top: h - half, left: w - half, cursor: 'nwse-resize' };
    case 's': return { top: h - half, left: w / 2 - half, cursor: 'ns-resize' };
    case 'sw': return { top: h - half, left: -half, cursor: 'nesw-resize' };
    case 'w': return { top: h / 2 - half, left: -half, cursor: 'ew-resize' };
  }
}
...
function renderContent(
  ...
  switch (type) {
    case 'text':
      return <TextElement data={data as never} elementId={elementId} spaceId={spaceId} isEditing={isEditing} onEditEnd={onEditEnd} />;
    case 'markdown':
...
function ShapeSvg({ type, w, h, data }: { type: string; w: number; h: number; data: Record<string, unknown> }) {
  ...
  switch (type) {
    ...
    case 'circle':
      return (
        <svg width={w} height={h}>
          <ellipse cx={w / 2} cy={h / 2} rx={Math.max(1, w / 2 - strokeWidth)} ry={Math.max(1, h / 2 - strokeWidth)}
...
function FlowchartNodeSvg({ type, w, h, data }: { type: string; w: number; h: number; data: Record<string, unknown> }) {
  ...
  const label = nodeData.label ?? (type === 'start-node' ? 'Start' : type === 'process-node' ? 'Process' : type === 'decision-node' ? 'Decision' : 'End');
  const sw = 2;
  ...
  const shape = (() => {
    switch (type) {
      case 'start-node':
      case 'end-node':
        return <rect x={sw / 2} y={sw / 2} width={w - sw} height={h - sw} rx={20} fill={fill} stroke={stroke} strokeWidth={sw} />;
      case 'process-node':
        return <rect x={sw / 2} y={sw / 2} width={w - sw} height={h - sw} rx={6} fill={fill} stroke={stroke} strokeWidth={sw} />;
      case 'decision-node':
        return (
          <polygon
...
function ArrowSvg({ element, isSelected, onPointDragStart, offsetX, offsetY, width, height }: {
  element: CanvasElement;
  isSelected: boolean;
  onPointDragStart: (e: React.MouseEvent, elementId: string, pointType: 'start' | 'end' | 'intermediate' | 'control', pointIndex: number) => void;
  offsetX: number;
  offsetY: number;
  width: number;
  height: number;
  ...
  const pts = useMemo(() => resolveArrowPoints(d).map(([x, y]) => [x - offsetX, y - offsetY] as [number, number]), [d, offsetX, offsetY]);
  const color = d.color ?? '#e2e8f0';
  const lw = d.lineWidth ?? 2;
  const style = d.arrowStyle ?? 'straight';
  const cp = d.controlPoint ? { x: d.controlPoint.x - offsetX, y: d.controlPoint.y - offsetY } : null;
  ...
  return (
    ...
    >
      ...
      <path d={path} fill="none" stroke={color} strokeWidth={lw}
        strokeLinecap="round" strokeLinejoin="round" />
      ...
      {isSelected && (
        <>
          <rect x={2} y={2} width={Math.max(0, width - 4)} height={Math.max(0, height - 4)}
            fill="none" stroke="#eab308" strokeWidth={2} rx={6} strokeDasharray="4 3"
            pointerEvents="none" />

          {pts.map((p, i) => (
            <circle key={i} cx={p[0]} cy={p[1]} r={5} fill="white" stroke="#eab308" strokeWidth={2}
              style={{ cursor: 'move' }}
              onMouseDown={(e) => {
                e.stopPropagation();
          ...
          {(() => {
            ...
            return (
              <circle cx={cx} cy={cy} r={4} fill="#eab308" stroke="white" strokeWidth={1.5}
                style={{ cursor: 'grab' }}
                onMouseDown={(e) => {
                  e.stopPropagation();
  ...
  return (
    ...
    >
      {isShape || isFlowchartNode ? (
        <>
          {isShape ? (
            <ShapeSvg type={element.type} w={element.size.width} h={element.size.height} data={element.data} />
          ) : (
            <FlowchartNodeSvg type={element.type} w={element.size.width} h={element.size.height} data={element.data} />
          )}
          <div
          ...
          />
          {isSelected && !element.locked && handlePositions.map(pos => (
            <div
              key={pos}
              data-handle-pos={pos}
