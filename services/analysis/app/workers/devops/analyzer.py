...
class DevopsAnalyzer:
    ...
    def _analyze_dockerfile(self, fp: str, content: str) -> list[DevOpsFinding]:
        ...
        is_dockerfile = "dockerfile" in fp or fp.endswith(".dockerfile")
        if not is_dockerfile:
            return findings
        lines = content.split("\n")
        has_healthcheck = bool(
            re.search(r"(?i)^\s*HEALTHCHECK\s", content, re.MULTILINE)
        )
        ...
        if has_add and not has_copy:
            for line_num, line in enumerate(lines, 1):
                if re.search(r"(?i)^\s*ADD\s", line) and not re.search(
                    r"(?i)\b(?:tar|gz|zip|url|https?://)\b", line
                ):
                    findings.append(
                        DevOpsFinding(
    ...
    def _analyze_docker_compose(self, fp: str, content: str) -> list[DevOpsFinding]:
        findings: list[DevOpsFinding] = []
        is_compose = bool(
            re.search(r"(?i)(?:docker-compose|compose)\.(?:yml|yaml)$", fp)
        )
        if not is_compose:
            return findings
        lines = content.split("\n")
    ...
    def _analyze_terraform(self, fp: str, content: str) -> list[DevOpsFinding]:
        ...
        if not is_tf:
            return findings
        if (
            "required_providers" not in content
            and "source  =" not in content
            and "version" not in content
        ):
            findings.append(
                DevOpsFinding(
        ...
        if "aws_s3_bucket" in content:
            has_versioning = bool(
                re.search(
                    r"(?i)versioning\s*\{[^}]*enabled\s*=\s*true", content, re.DOTALL
                )
            )
    ...
    def _analyze_helm(self, fp: str, content: str) -> list[DevOpsFinding]:
        ...
        if (
            fp.endswith("values.yaml")
            or fp.endswith("templates/")
            or "/templates/" in fp
        ) and not re.search(r"\{\{\s*\.Values\.", content):
            findings.append(
                DevOpsFinding(
        ...
        if "namespace:" in content and (
            "default"
            in content.split("namespace:")[1].split("\n")[0].strip().strip('"')
        ):
            findings.append(
                DevOpsFinding(
        ...
        if "containers:" in content:
            ...
            for line_num, line in enumerate(content.split("\n"), 1):
                if re.search(r"(?i)privileged\s*:\s*true", line):
                    ...
                    break
        if (
            "PodDisruptionBudget" not in content
            and "poddisruptionbudget" not in content.lower()
        ):
    ...
    def _analyze_nginx(self, fp: str, content: str) -> list[DevOpsFinding]:
        ...
        if (
            "ssl_certificate" not in full_lower
            and "ssl_certificate_key" not in full_lower
        ):
            findings.append(
                DevOpsFinding(
        ...
        if (
            "X-Content-Type-Options" not in content
            and "X_CONTENT_TYPE_OPTIONS" not in content
        ):
    ...
    def _analyze_observability(self, fp: str, content: str) -> list[DevOpsFinding]:
        ...
        full_lower = content.lower()
        has_metrics = bool(
            re.search(r"(?i)(?:metrics|/metrics|prometheus)", full_lower)
        )
        has_grafana = bool(re.search(r"(?i)(?:grafana|dashboard)", full_lower))
        has_tracing = bool(
            re.search(r"(?i)(?:opentelemetry|opentracing|otel|jaeger)", full_lower)
        )
        has_structured_logging = bool(
            re.search(
                r"(?i)(?:json|structured|logstash)\s*(?:format|formatter)", full_lower
            )
        )
        has_log_agg = bool(
            re.search(
                r"(?i)(?:filebeat|logstash|fluentd|fluent\s*bit|datadog)", full_lower
            )
        )
        has_any = (
            has_metrics
    ...
    def _analyze_health(self, fp: str, content: str) -> list[DevOpsFinding]:
        ...
        has_health_endpoint = bool(
            re.search(
                r'(?i)(?:["\']/health["\']|["\']/healthz["\']|["\']/ready["\']|["\']/live["\']|health_check|healthcheck)',
                content,
            )
        )
        has_readiness = bool(re.search(r"(?i)readinessProbe\s*:", content))
    ...
    def _analyze_env(self, fp: str, content: str) -> list[DevOpsFinding]:
        ...
        secrets_pattern = re.compile(
            r'(?im)^\s*(?:SECRET|SECRET_KEY|API_KEY|PASSWORD|TOKEN|PRIVATE_KEY|ACCESS_KEY)\s*[=:]\s*["\']?[^"\'${\s]+["\']?\s*$'
        )
        has_secrets = False
        if ".gitignore" in fp:
            ...
            if not has_secrets and "example" not in fp and "template" not in fp:
                has_docs = bool(
                    re.search(
                        r"(?i)#\s*(?:required|needed|mandatory|document)", content
                    )
                )
                if not has_docs:
                    findings.append(
    ...
    def _analyze_operations(self, fp: str, content: str) -> list[DevOpsFinding]:
        ...
        has_autoscaling = bool(
            re.search(
                r"(?i)(?:autoscal|hpa\b|horizontalpodautoscal|horizontal.*autoscale|auto.scale)",
                full_lower,
            )
        )
        has_dr = bool(
            re.search(
                r"(?i)(?:disaster.recovery|dr\s*plan|failover|multi.region|region.replication)",
                full_lower,
            )
        )
        has_rollback = bool(
            re.search(
                r"(?i)(?:rollback|blue.green|canary|gradual.rollout|deployment.strategy)",
                full_lower,
            )
        )
        has_iac_quality = bool(
            re.search(
                r"(?i)(?:terraform.fmt|tfsec|checkov|terragrunt|terrascan)", full_lower
            )
        )
        has_any = (
            has_backup or has_autoscaling or has_dr or has_rollback or has_iac_quality
        )
