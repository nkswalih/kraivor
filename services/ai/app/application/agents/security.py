...
class SecurityAnalystNode:
    ...
    async def __call__(self, state: dict) -> dict:
        ...
        if state.get("provider_error"):
            logger.info("node=security_analyst skipped — prior provider_error")
        ...
        try:
            ...
            logger.warning("provider_error node=security_analyst category=%s", e.category.value)
            return {
                "security_findings": [],
        ...
        logger.info(
            "node=security_analyst elapsed=%.2fs tokens_out=%d model=%s",
