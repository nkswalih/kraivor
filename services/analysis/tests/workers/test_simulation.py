        ...
        metrics = PerformanceMetrics(
            endpoints=[
                EndpointMetric(
                    ...
                    estimated_rpm=2000, p50_latency_ms=10,
                    p95_latency_ms=25, p99_latency_ms=50,
                    max_concurrent_users=10000,
                ),
            ],
            overall_rpm=2000, breaks_at_concurrent_users=10000,
        )
        ...
        metrics = PerformanceMetrics(
            endpoints=[
                EndpointMetric(
                    ...
                    estimated_rpm=100, p50_latency_ms=300,
                    p95_latency_ms=500, p99_latency_ms=1000,
                    max_concurrent_users=500,
                    bottlenecks=["n_plus_one"],
                ),
            ],
            overall_rpm=100, breaks_at_concurrent_users=500,
            bottlenecks=["n_plus_one"],
        )
        simulator = ProductionSimulator()
        results = await simulator.simulate(metrics)
    ...
    async def test_failing_at_high_load(self) -> None:
        metrics = PerformanceMetrics(
            endpoints=[
                EndpointMetric(
                    endpoint="/api/slow", method="GET",
                    estimated_rpm=50, p50_latency_ms=1000,
                    p95_latency_ms=2500, p99_latency_ms=5000,
                    max_concurrent_users=100,
                    bottlenecks=["n_plus_one", "sync_external_call"],
                ),
            ],
            overall_rpm=50, breaks_at_concurrent_users=100,
            bottlenecks=["n_plus_one", "sync_external_call"],
        )
        simulator = ProductionSimulator()
        results = await simulator.simulate(metrics)
        ...
        metrics = PerformanceMetrics(
            endpoints=[
                EndpointMetric(
                    ...
                    estimated_rpm=500, p50_latency_ms=100,
                    p95_latency_ms=250, p99_latency_ms=500,
                    max_concurrent_users=2000,
                    bottlenecks=["many_db_queries"],
                ),
            ],
            overall_rpm=500, breaks_at_concurrent_users=2000,
        )
        simulator = ProductionSimulator()
        results = await simulator.simulate(metrics)
    ...
    async def test_bottlenecks_propagate(self) -> None:
        metrics = PerformanceMetrics(
            endpoints=[
                EndpointMetric(
                    endpoint="/api/test", method="GET",
                    estimated_rpm=300, p50_latency_ms=150,
                    p95_latency_ms=375, p99_latency_ms=750,
                    max_concurrent_users=500,
                    bottlenecks=["sync_external_call", "n_plus_one"],
                ),
            ],
            overall_rpm=300, breaks_at_concurrent_users=500,
        )
        simulator = ProductionSimulator()
        results = await simulator.simulate(metrics)
        for r in results:
            if r.bottlenecks:
                assert "n_plus_one" in r.bottlenecks or "sync_external_call" in r.bottlenecks or "increased_load" in r.bottlenecks
