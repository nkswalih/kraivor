...
def list_discussions_by_author(
    author_id: uuid.UUID | str, page: int = 1, page_size: int = 20
):
...
def list_comments_by_author(
    author_id: uuid.UUID | str, page: int = 1, page_size: int = 30
):
    qs = Comment.objects.filter(
        author_id=author_id, parent__isnull=True
    ).select_related(None)
    total = qs.count()
    offset = (page - 1) * page_size
    items = qs[offset : offset + page_size]
