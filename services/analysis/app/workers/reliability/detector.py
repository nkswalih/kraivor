...
class ReliabilityDetector:
        ...
        for line_num, line in enumerate(lines, 1):
            for pattern_str in external_call_patterns:
                if re.search(pattern_str, line):
                    nearby_lines = lines[
                        max(0, line_num - 6) : min(len(lines), line_num + 2)
                    ]
    ...
    def _detect_missing_circuit_breaker(
        self, file_path: str, content: str
    ) -> list[ReliabilityFinding]:
    ...
    def _detect_transaction_misuse(
        self, file_path: str, content: str
    ) -> list[ReliabilityFinding]:
        ...
        for line_num, line in enumerate(lines, 1):
            if re.search(r"(?i)(?:begin|start)_(?:transaction|session)", line):
                surrounding = "\n".join(
                    lines[max(0, line_num - 2) : min(len(lines), line_num + 15)]
                )
    ...
    def _detect_resource_leaks(
        ...
        patterns = [
            (
                r"(?i)open\s*\([^)]*\)(?!\s*as\s)",
                "File handle may not be closed — use 'with open() as f:'",
            ),
            (
                r"(?i)(?:create_connection|connect)\s*\([^)]*\)(?!\s*as\s)",
                "Connection may not be closed — use context manager",
            ),
            (
                r"(?i)(?:StringIO|BytesIO)\s*\([^)]*\)(?!\s*as\s)",
                "I/O stream may not be closed — use context manager",
            ),
        ]
        lines = content.split("\n")
        for line_num, line in enumerate(lines, 1):
    ...
    def _detect_infinite_loops(
        ...
        for line_num, line in enumerate(lines, 1):
            if re.search(r"(?i)while\s+True\s*:", line):
                surrounding = "\n".join(
                    lines[line_num : min(len(lines), line_num + 20)]
                )
    ...
    def _detect_thread_safety(
        ...
        if not has_sync:
            ...
            for line_num, line in enumerate(lines, 1):
                if re.search(
                    r"(?i)(?:threading|ThreadPoolExecutor|multiprocessing)", line
                ):
                    findings.append(
                        ReliabilityFinding(
    ...
    def _detect_cache_stampede(
        ...
        for line_num, line in enumerate(lines, 1):
            if re.search(
                r"(?i)(?:cache|redis|memcached)\.(?:get|fetch|retrieve)\s*\(", line
            ):
                surrounding = "\n".join(
                    lines[max(0, line_num - 3) : min(len(lines), line_num + 5)]
                )
                has_ttl = any(
    ...
    def _detect_idempotency(
        ...
        for r in routes:
            if not re.search(
                r"(?i)(?:GET|get|list|fetch|read|query)", str(r.get("text", ""))
            ):
                mutating_count += 1
    ...
    def _detect_connection_pool_misuse(
        self, file_path: str, content: str
    ) -> list[ReliabilityFinding]:
        ...
        lines = content.split("\n")
        for line_num, line in enumerate(lines, 1):
            if re.search(r"(?i)def\s+\w+\(.*request.*\):", line) or re.search(
                r"(?i)@app\.route|@router\.", line
            ):
                surrounding = "\n".join(
                    lines[line_num : min(len(lines), line_num + 15)]
                )
                has_pool = any(
                    kw in surrounding.lower()
                    for kw in (
                ...
                has_new_connection = bool(
                    re.search(
                        r"(?i)(?:create_connection|new\s+connection|connect\s*\(|psycopg2\.connect|sqlite3\.connect)",
                        surrounding,
                    )
                )
    ...
    def _detect_stale_cache(
        ...
        for line_num, line in enumerate(lines, 1):
            if re.search(
                r"(?i)(?:cache|redis|memcached)\.(?:set|put|store|add)\s*\(", line
            ):
                has_ttl = (
                    "ttl" in line.lower()
    ...
    def _detect_dead_letter(
        ...
        for line_num, line in enumerate(lines, 1):
            if re.search(
                r"(?i)(?:def\s+\w+.*event|def\s+\w+.*message|def\s+\w+.*handle_\w+)",
                line,
            ):
                surrounding = "\n".join(
                    lines[line_num : min(len(lines), line_num + 10)]
                )
                has_dead_letter = any(
                    kw in surrounding.lower()
                    for kw in (
