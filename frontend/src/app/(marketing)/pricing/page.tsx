...
const Icons = {
  Check: (props: React.SVGProps<SVGSVGElement>) => (
    <svg {...props} width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round">
      <path d="M4.5 12.75l6 6 9-13.5" />
    </svg>
  ),
  ArrowRight: (props: React.SVGProps<SVGSVGElement>) => (
    <svg {...props} width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
        ...
        <section className="container mx-auto px-6 pt-12 pb-20 text-center">
          <div className="animate-fade-up mx-auto max-w-3xl">
            ...
            <p className="text-xl text-slate-400 mb-10">
              Choose the plan that fits your team's needs. Scale your intelligence, pay for what you use.
            </p>
          ...
          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            {PRICING_TIERS.map((tier, idx) => (
              <RevealSection key={idx} delay={`delay-[${idx * 100}ms]`}>
                <div className={`group relative h-full rounded-3xl border p-8 backdrop-blur-xl transition-all duration-300 hover:-translate-y-2 
                  ...
                  <div className="mb-5 flex items-center justify-between">
                    <h3 className={`text-2xl font-bold ${tier.highlight ? 'text-[hsl(var(--primary-light))]' : 'text-white group-hover:text-[hsl(var(--primary-light))]'} transition-colors`}>{tier.name}</h3>
                    {tier.popular && (
                      <span className="rounded-full bg-[hsl(var(--primary))]/20 px-3 py-1 text-xs font-medium text-[hsl(var(--primary-light))]">Save 20%</span>
                    )}
                  </div>
                  ...
                  <div className="mb-8 border-b border-white/10 pb-6">
                    <p className="flex items-baseline gap-1 justify-center">
                      <span className={`text-5xl font-extrabold ${tier.highlight ? 'text-[hsl(var(--primary-light))]' : 'text-white group-hover:text-[hsl(var(--primary-light))]'} transition-colors`}>
                        {formatCurrency(tier.price[billingMode])}
                      </span>
                      <span className="text-sm font-medium text-slate-400">/{billingMode === 'annually' ? 'year' : 'month'}</span>
                    </p>
                    {billingMode === 'monthly' && (
                  ...
                  <ul className="space-y-4 text-sm text-slate-300">
                    {tier.features.map((feature, i) => (
                      <li key={i} className="flex items-center gap-3">
                        <div className={`flex h-6 w-6 items-center justify-center rounded-full border transition-colors ${tier.highlight ? 'bg-[hsl(var(--primary))]/20 border-[hsl(var(--primary))/40]' : 'bg-white/5 border-white/10 group-hover:bg-white/10 group-hover:border-[hsl(var(--primary))/30]'}`}>
                          <Icons.Check className={`${tier.highlight ? 'text-[hsl(var(--primary-light))]' : 'text-slate-300 group-hover:text-[hsl(var(--primary-light))]'} transition-colors`}/>
                        </div>
                        <span>{feature}</span>
                      </li>
                    ))}
                  ...
                  <Link href={ROUTES.REGISTER} className={`mt-10 block w-full rounded-xl px-6 py-3 text-center text-base font-semibold transition-all duration-300 
                    ${tier.highlight 
            ...
            <div className="rounded-3xl border border-white/10 bg-white/[0.01] p-8 md:p-12 backdrop-blur-sm">
              ...
              <div className="grid gap-6 md:grid-cols-2">
                {[
                  ...
                  { question: "Can I switch plans later?", answer: "Yes, you can upgrade or downgrade your plan at any time. Your billing will be prorated accordingly." },
                  ...
                  { question: "Do you offer custom enterprise solutions?", answer: "Absolutely. Contact our sales team for custom integrations, dedicated support, and tailored solutions for your organization's unique needs." },
                  { question: "How does the async processing work?", answer: "Any operation over 500ms is handled in the background. Results are pushed via WebSocket or polled, ensuring immediate HTTP responses and 10,000+ concurrent users without slowdowns." },
                  { question: "What is the refund policy?", answer: "We offer a 14-day money-back guarantee on all plans. If you're not satisfied, you can request a full refund within 14 days of your purchase." },
                ].map((faq, i) => (
      ...
      <footer className="border-t border-white/10 bg-[#0a0a0f] py-12">
        <div className="container mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-6">
            <Link href="#" className="text-sm text-slate-500 hover:text-white transition-colors">Status</Link>
            <Link href="#" className="text-sm text-slate-500 hover:text-white transition-colors">API Docs</Link>
