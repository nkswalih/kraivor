...
class TestProjectListCreateView:
    ...
    def test_list_returns_200(self, authed_client, workspace, project, workspace_member):
        resp = authed_client.get(f"/api/workspaces/{workspace.id}/projects/")
        assert resp.status_code == 200
        assert len(resp.data) == 1

    def test_list_excludes_other_workspace(self, authed_client, workspace, workspace_member):
        ProjectFactory()
        resp = authed_client.get(f"/api/workspaces/{workspace.id}/projects/")
        assert len(resp.data) == 0

    def test_list_filter_by_status(self, authed_client, workspace, user_id, workspace_member):
        ProjectFactory(workspace=workspace, status="active")
        ProjectFactory(workspace=workspace, status="planning")
        resp = authed_client.get(f"/api/workspaces/{workspace.id}/projects/?status=active")
        assert len(resp.data) == 1

    def test_create_returns_201(self, authed_client, workspace, workspace_member):
        resp = authed_client.post(f"/api/workspaces/{workspace.id}/projects/", {"name": "New Project"})
...
class TestProjectDetailView:
    ...
    def test_get_returns_200(self, authed_client, workspace, project, workspace_member):
        resp = authed_client.get(f"/api/workspaces/{workspace.id}/projects/{project.id}/")
        assert resp.status_code == 200
    ...
    def test_get_wrong_workspace_returns_404(self, api_client):
        ...
        resp = api_client.get(f"/api/workspaces/{wrong_workspace.id}/projects/{other_project.id}/")
        assert resp.status_code == 404

    def test_patch_returns_200(self, authed_client, workspace, project, workspace_member):
    ...
    def test_delete_returns_204(self, authed_client, workspace, project, workspace_member):
        resp = authed_client.delete(f"/api/workspaces/{workspace.id}/projects/{project.id}/")
        assert resp.status_code == 204

    def test_deleted_project_not_in_list(self, authed_client, workspace, project, workspace_member):
...
class TestTaskStatusUpdateView:
    ...
    def test_update_status_returns_200(self, authed_client, workspace, task, workspace_member):
        resp = authed_client.patch(
    ...
    def test_update_invalid_status_returns_400(self, authed_client, workspace, task, workspace_member):
        resp = authed_client.patch(
            f"/api/workspaces/{workspace.id}/tasks/{task.id}/status/",
            {"status": "flying"},
        )
...
class TestTaskDependencyView:
    ...
    def test_add_dependency_returns_201(self, authed_client, workspace, project, workspace_member):
    ...
    def test_add_self_dependency_returns_400(self, authed_client, workspace, task, workspace_member):
        resp = authed_client.post(
    ...
    def test_remove_dependency_returns_204(self, authed_client, workspace, project, workspace_member):
        t1 = TaskFactory(project=project)
        t2 = TaskFactory(project=project)
        link = TaskLink.objects.create(
            source_task=t1, target_task=t2,
            relationship_type="blocks", created_by=uuid.uuid4()
        )
