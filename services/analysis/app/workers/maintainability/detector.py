...
class MaintainabilityDetector:
    ...
    def compute_metrics(self, findings: list[MaintainabilityFinding]) -> MaintainabilityMetrics:
        total = len(findings)
        if total == 0:
        ...
        complexity_violations = sum(
            1 for f in findings if f.maintainability_type in (
                "high_cyclomatic_complexity", "deep_nesting", "deep_inheritance",
            )
        )
        ...
        technical_debt_hours = sum(
            _EFFORT_ESTIMATES.get(f.maintainability_type, 0.0)
            for f in findings
        )
        ...
        for pf in self.parsed_files:
            for func in pf.functions:
                total_complexity += func.complexity if hasattr(func, 'complexity') else 0
        ...
        for func in pf.functions:
            if hasattr(func, 'line_start') and hasattr(func, 'line_end'):
                ...
                if length > 50:
                    ...
                    snippet_lines = content_lines[max(0, func.line_start - 1):min(len(content_lines), func.line_start + 2)]
    ...
    def _detect_too_many_parameters(self, pf: ParsedFile) -> list[MaintainabilityFinding]:
        findings: list[MaintainabilityFinding] = []
        content_lines = pf.content.split("\n")
        for func in pf.functions:
            ...
            if len(params) > 5:
                snippet = content_lines[min(start, len(content_lines) - 1)].strip() if content_lines else ""
                findings.append(MaintainabilityFinding(
    ...
    def _detect_high_cyclomatic_complexity(self, pf: ParsedFile) -> list[MaintainabilityFinding]:
        findings: list[MaintainabilityFinding] = []
        for func in pf.functions:
            ...
            if complexity > 10:
                content_lines = pf.content.split("\n")
                snippet = ""
                if content_lines and func.line_start > 0 and func.line_start <= len(content_lines):
                ...
                findings.append(MaintainabilityFinding(
                    ...
                    confidence=0.9,
                    estimated_effort_hours=_EFFORT_ESTIMATES["high_cyclomatic_complexity"],
                ))
        return findings

    def _detect_deep_nesting(self, file_path: str, content: str) -> list[MaintainabilityFinding]:
        findings: list[MaintainabilityFinding] = []
        lines = content.split("\n")
        ...
        max_depth = 0
        max_depth_line = 0
        nesting_keywords = re.compile(r'(if |elif |else:|for |while |with |try:|except |finally:)')
    ...
    def _detect_duplicate_code(self, file_path: str, content: str) -> list[MaintainabilityFinding]:
        findings: list[MaintainabilityFinding] = []
        lines = content.split("\n")
        ...
        for i in range(len(lines) - 2):
            ...
            if len(block) < 20 or block.startswith("#") or block.startswith("import") or block.startswith("from"):
                continue
    ...
    def _detect_large_class(self, pf: ParsedFile) -> list[MaintainabilityFinding]:
        ...
        for cls in pf.classes:
            ...
            if length > 300:
                content_lines = pf.content.split("\n")
                snippet = ""
                if content_lines and cls.line_start > 0 and cls.line_start <= len(content_lines):
    ...
    def _detect_long_lines(self, file_path: str, content: str) -> list[MaintainabilityFinding]:
        findings: list[MaintainabilityFinding] = []
        lines = content.split("\n")
        for line_num, line in enumerate(lines, 1):
            if len(line) > 120:
    ...
    def _detect_missing_docstrings(self, pf: ParsedFile, file_path: str) -> list[MaintainabilityFinding]:
        findings: list[MaintainabilityFinding] = []
        for func in pf.functions:
    ...
    def _detect_magic_numbers(self, file_path: str, content: str) -> list[MaintainabilityFinding]:
        findings: list[MaintainabilityFinding] = []
        lines = content.split("\n")
    ...
    def _detect_empty_catch_blocks(self, file_path: str, content: str) -> list[MaintainabilityFinding]:
        findings: list[MaintainabilityFinding] = []
        lines = content.split("\n")
        for line_num, line in enumerate(lines, 1):
            stripped = line.strip()
            if re.match(r'^except\s*:', stripped) or re.match(r'^except\s+\w+\s*:', stripped):
                ...
                if "pass" in next_lines[:20]:
                    findings.append(MaintainabilityFinding(
                        ...
                        confidence=0.9,
                        estimated_effort_hours=_EFFORT_ESTIMATES["empty_catch_block"],
                    ))
        return findings

    def _detect_todo_comments(self, file_path: str, content: str) -> list[MaintainabilityFinding]:
        findings: list[MaintainabilityFinding] = []
        lines = content.split("\n")
    ...
    def _detect_deep_inheritance(self, pf: ParsedFile, file_path: str) -> list[MaintainabilityFinding]:
        findings: list[MaintainabilityFinding] = []
        for cls in pf.classes:
    ...
    def _detect_circular_imports(self, file_path: str, content: str) -> list[MaintainabilityFinding]:
        findings: list[MaintainabilityFinding] = []
        lines = content.split("\n")
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if re.match(r'^def\s+\w+\s*\(', stripped) or re.match(r'^async\s+def\s+\w+\s*\(', stripped):
