...
export const chatEndpoints = {
  ...
  updateRoom: (
    workspacePk: string,
    roomPk: string,
    payload: Partial<Pick<ChatRoom, 'name' | 'topic'>>
  ...
  listMessages: (workspacePk: string, roomPk: string, limit = 50, startKey?: string) => {
    ...
    return coreApi.get<CursorPage<ChatMessage>>(
      `/workspaces/${workspacePk}/chat/rooms/${roomPk}/messages/?${params}`
    );
  },
