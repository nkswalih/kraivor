...
def create_api_key(
    user: User, name: str, scopes: list[str], expires_at=None
) -> CreatedAPIKey:
    """
...
def revoke_api_key(user: User, key_id: str) -> APIKey:
    ...
    logger.info(
        "api_key_revoked: user_id=%s key_id=%s name=%r", user.id, key.id, key.name
    )
...
def authenticate_api_key(raw_key: str) -> APIKey:
    ...
    logger.info(
        "api_key_auth_success: key_id=%s user_id=%s", api_key.id, api_key.user_id
    )
